...
async def process_file(message, prompt, model_to_use, file_type, status_msg, display_prompt=False):
    """Processes files (image, audio, video) and sends them to Generative AI."""
    ...
    try:
        ...
        # Handle audio or video files
            ...
            while uploaded_file.state.name == "PROCESSING":
                await message.edit_text("<code>Processing file on server...</code>")
                time.sleep(10)
                uploaded_file = genai.get_file(uploaded_file.name)

            if uploaded_file.state.name == "FAILED":
                return await message.edit_text("<code>File upload failed. Please try again.</code>")
...
async def transcribe(_, message):
    ...
    await process_file(message, prompt, model, "audio", "Transcribing audio or video...", len(message.command) > 1)
...
modules_help["generative"] = {
    ...
    "transcribe [custom prompt] [reply to audio]*": "Transcribe or describe an audio or video message.",
}
